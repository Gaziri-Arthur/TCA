...
/* O R I E N T A S   E X C L U I   E   E D I T A */
void orientaeditaamigo();                                    // edita dados dos amigos
...
void orientaeditacategoria();                                // orienta a edicao de categoria
...
/* E D I T A S   E   E X C L U I S */
void editaamigo(char editor, int amigo);                     // factualmente muda os dados do amigo
...
void editacategoria(int cat);                                // edita categorias
...
/* V A L I D A C O E S   D E   D A D O S */
...
bool validanomelocal(char *nome);                            // checks for doubles
bool validacat(char *str);                                   // checks for doubles

/* E X I B I C A O   D E   D A D O S   D I S P O N I V E I S */
int showchavfr(tAmigo *vet[], int t);                        // exibe os amigos disponiveis para a incusao num encontro
int showchavcat(tEncontro encontro);                         // exibe as categorias disponiveis
int showchavpl(tEncontro encontro);                          // exibe os locais disponiveis
...
int buscadadovalido(int indice, int delimitador);            // busca por indices de qualquer dado

/* M A N I P U L A C O E S   D E   M E M O R I A */
...
void limpaamigo();                                           // dos amigos
void limpalocal();                                           // dos locais
void limpacategoria();                                       // das categorias
void limpaencontro();                                        // dos encontros
...
void excluiencontro(int op)
{
    ...
    {
        Listaencontro[op].amigos[i] = NULL;
        free(Listaencontro[op].idamigos[i]);
    }
    free(Listaencontro[op].amigos);
...
void editaamigosencontro(int op, int encontro)
{
    ...
    case 1:
        if (Numamigo == Listaencontro[encontro].Namigos)
        {
            ...
            printf("Nao existem amigos para serem adicionados.\nDeseja criar um novo amigo?(s/n): ");
            if (!validasimnao())
            {
                CLS return;
            }
    ...
    case 2:
        ...
        printf("\n\nQual amigo deseja excluir?(indice): ");
        (i = opvalido(Listaencontro[encontro].Namigos) - 1);
...
void rereferenciaamigo()
{
    ...
    for (int i = 0; i < Numencontro; i++)
    {
        ...
        for (int j = 0; j < Listaencontro[i].Namigos; j++)
        {
            ...
            {
                printf("ERRO: ID %s nao encontrado em Listaamigo.\n", Listaencontro[i].idamigos[j]);
                exit(1);
            }
        }
    }
...
void enviaencontro()
{
    ...
    for (int i = 0; i < Numencontro; i++)
    {
        ...
        fprintf(Ponta, "%s\n%s\n", Listaencontro[i].local->nomelocal, Listaencontro[i].categoria->categoria);
        fprintf(Ponta, "%d;\n", Listaencontro[i].Namigos);
        for (int j = 0; j < Listaencontro[i].Namigos; j++)
        {
            fprintf(Ponta, "%s\n", Listaencontro[i].amigos[j]->nome);
        }
    }
    fclose(Ponta);
    return;
...
tEncontro criaencontro()
{
    ...
    {
        printf("--------------------------------\nDigite:\n\n[1] para adicionar um local existente.\n[2] para adicionar um novo local.\n\n");
        printf("--Opcao desejada: ");
        char op = valida12();
        if (op == '1')
        {
            exibelocal(true, false);
            printf("\n\nQual local deseja adicionar?(nome/indice): ");
            int opl = buscadadovalido(2, Numlocal);
    ...
    else
    {
        ...
        if (op == 1)
        {
            ...
            printf("\n\nQual amigo deseja adicionar?(nome/indice): ");
            int opl = buscadadovalido(1, Numamigo);
    ...
    do // adicao de novos amigos
    {
        ...
        CLS if (op)
        {
            ...
            else
            {
                ...
                encontro.idamigos = (char **)realloc(encontro.idamigos, (encontro.Namigos + 1) * sizeof(char *));
                verifica_alocacao(encontro.idamigos);
                encontro.idamigos[encontro.Namigos] = (char *)malloc((strlen(encontro.amigos[encontro.Namigos]->nome) + 1) * sizeof(char));
                verifica_alocacao(encontro.idamigos[encontro.Namigos]);
    ...
    else
    {
        ...
        if (op == 1)
        {
            ...
            printf("\n\nQual categoria deseja adicionar?(nome/indice): ");
            int opl = buscadadovalido(3, Numcategoria);
...
void exibelocal(bool prolongar, bool rel)
{
    ...
    else if (prolongar)
    {
        ...
        for (int i = 0; i < Numlocal; i++)
        {
            printf("Local [%d]:\nNome: %s.\nEndereco: %s, %s.\nBairro: %s.\nCidade: %s.\nEstado: %s.\n\n", i + 1, Listalocal[i].nomelocal, Listalocal[i].endereco.numero, Listalocal[i].endereco.logradouro, Listalocal[i].endereco.bairro, Listalocal[i].endereco.cidade, Listalocal[i].endereco.estado);
            printf("------------------------------------------\n\n");
        }
    }
...
void switchrelatorios(char op)
{
    ...
    case '4':
        ...
        else
        {
            ...
            {
                exibeencontro(false, false, false);
                printf("\n\nQual encontro deseja exibir?(nome/indice): ");
    ...
    case '5':
        ...
        else
        {
            ...
            relpercat(buscadadovalido(3, Numcategoria));
        }
        break;
    }
...
void editaamigo(char editor, int amigo)
{
    ...
    for (int j = 0; j < Numencontro; j++)
    {
        for (int k = 0; k < Listaencontro[j].Namigos; k++)
        {
            if (strcmp(Listaencontro[j].amigos[k]->nome, Listaamigo[i].nome) == 0)
            {
                ...
                if (validasimnao())
                {
                    CLS if (Listaencontro[j].Namigos > 1)
                    {
...
bool validaapelido(char *apelido)
{
    for (int i = 0; i < Numamigo; i++)
    {
        if (strcmp(apelido, Listaamigo[i].apelido) == 0 || strcmp(apelido, Listaamigo[i].nome) == 0)
        {
            return false;
        }
    }
...
tAmigo criaamigo()
{
    ...
    do
    {
        ...
        if(strcmp(str, amigo.nome) == 0)
        {
            printf("\nSeu amigo nao pode ter o mesmo apelido que seu nome.\nDigite novamente: ");
        }
        else
        {
...
bool validanome(char *nome)
{
    ...
    for (int i = 0; i < Numamigo; i++)
    {
        if (strcmp(nome, Listaamigo[i].nome) == 0 || strcmp(nome, Listaamigo[i].apelido) == 0)
        {
            return false;
        }
    }
